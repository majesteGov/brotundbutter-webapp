...
public class AppointmentServlet extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (date1 != null && !date1.isEmpty() && heure1 != null && !heure1.isEmpty() && date2 != null && !date2.isEmpty() && heure2 != null && !heure2.isEmpty() && vaccine != null && !vaccine.isEmpty()) {
...
//Verification si la date entre n'est pas dans le passe
                ...
                if (diff < 14) {
                    ...
                    response.getWriter().println("<br><h2>Das Datum des zweiten Termins muss mindestens 14 Tage nach dem Datum des ersten Termins liegen.</h2>");
            ...
            try {
                ...
                PreparedStatement stmt1 = connection.prepareStatement("SELECT COUNT(*) FROM appointmentsApp WHERE date1=? AND heure1=?");
                ...
                if (count1 >= 2) {
                    response.getWriter().println("<br><h2>Die für den ersten Termin gewählte Uhrzeit ist nicht mehr verfügbar. Bitte wählen Sie eine andere.<h2>");
                    return;
                }

                // Compter le nombre de rendez-vous pour l'heure 2 a la date 2
                PreparedStatement stmt2 = connection.prepareStatement("SELECT COUNT(*) FROM appointmentsApp WHERE date2=? AND heure2=?");
                ...
                if (count2 >= 2) {
                    ...
                    response.getWriter().println("<br><h2>Die für den zweiten Termin gewählte Uhrzeit ist nicht mehr verfügbar. Bitte wählen Sie eine andere.</h2>");
                    return;
                }
                ...
                // Compter le nombre de rendez-vous pour l'heure 1 a la date 1
                PreparedStatement stmt3 = connection.prepareStatement("SELECT COUNT(*) FROM appointmentsApp WHERE date1=? AND heure1=? AND username=?");
                ...
                if (count1 > 0) {
                    ...
                    response.getWriter().println("Sie haben bereits einen Termin für die Uhrzeit des ersten gewählten Zeitfensters gebucht.");
                    return;
                }

                // Compter le nombre de rendez-vous pour l'heure 2 a la date 2
                PreparedStatement stmt4 = connection.prepareStatement("SELECT COUNT(*) FROM appointmentsApp WHERE date2=? AND heure2=? AND username=?");
                ...
                // Verifier si l'utilisateur connecte a deja reserve ce creneau horaire
                if (count2 > 0) {
                    response.getWriter().println("Sie haben bereits einen Termin für die Uhrzeit des zweiten gewählten Zeitfensters gebucht.");
                    return;
                }

                // Enregistrement du rendez-vous dans la base de donnees
                PreparedStatement stmt5 = connection.prepareStatement("INSERT INTO appointmentsApp (username, first_name, last_name, email, city, postal_code, date1, heure1, date2, heure2, vaccine) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                ...
                if (rowsAffected > 0) {
                    ...
                    response.getWriter().println("Ihr Termin konnte nicht gespeichert werden. Bitte versuchen Sie es zu einem späteren Zeitpunkt erneut.");
                }
                connection.close();
            } catch (Exception e) {
